...
public class Poly {
    ...
    private Map<Mono.Sign, Mono> monos = new HashMap<>();
    ...
    public Poly negate() {
        ...
        for (Mono mono : monos.values()) {
            ans.addMono(new Mono(
                mono.getCoefficient().negate(), mono.getExponent(), mono.getExpPoly()
            ));
        }
        return ans;
    }
    ...
    public Poly multiply(Poly other) {
        ...
        Poly ans = new Poly(Math.max(16, this.monos.size() * other.monos.size() / 2));
    ...
    public boolean equals(Object obj) {
        ...
        for (Map.Entry<Mono.Sign, Mono> entry : this.monos.entrySet()) {
    ...
    public String toOptimalString(boolean isNested) {
        ...
        if (firstPositive != -1) {
            sb.append(list.get(firstPositive).toOptimalString(isNested));
            isFirst = false;
        }
        for (int i = 0; i < list.size(); i++) {
            if (i != firstPositive && !list.get(i).isZero()) {
                String term = list.get(i).toOptimalString(isNested);
                if (term.isEmpty()) {
                    continue;
                }
                if (isFirst) {
