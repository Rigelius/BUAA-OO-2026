...
本次作业需要完成的任务为：读入一个包含加、减、乘、乘方以及括号（其中括号的深度**至多为 3 层**）的**单变量**表达式，输出**恒等变形展开所有括号后**的表达式。
...
* **空白字符** 在本次作业中，空白字符包含且仅包含空格 `<space>`（ascii 值 32）和水平制表符 `\t`（ascii 值 9）。其他的空白字符，均属于非法字符。
  ...
  对于空白字符，有以下几点规定：
    ...
    + 因子、项、表达式，在不与以上条件矛盾的前提下，可以在任意位置包含任意数量的空白字符。
...
- 本次作业中对于恒等的定义：设与 $x$ 有关的表达式 $f(x)$ 的定义域为 $D_1$，$D_1$ 包含于 $R$，$g(x)$ 的定义域为 $D_2$，$D_2$ 包含于 $R$，若对任意 $x\in D_1\cap D_2$，$f(x)=g(x)$ 成立，则 $f(x)$ 与 $g(x)$ 恒等。
...
4. 同质测试用例：
   ...
   - 具体分数计算根据bug修复情况调整，同质bug是在bug修复基础上做出的裁定。
...
* 若在架构设计中遇到困难，可以参考面向对象设计与构造课程组官方公众号往年相关文章 [OO加油站——U1任务迭代](https://mp-weixin-qq-com-443.e1.buaa.edu.cn/s/3SsmLo_MyTgyQAhL01-g0A)
