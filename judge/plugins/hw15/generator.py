#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""HW15 testcase generator for the library judge."""
...
MAX_COMMANDS = 3000
FIXED_CASES_DIR = Path(__file__).resolve().parent / "fixed_cases_hw15"
...
class CaseBuilder:
    ...
    def random_request(self) -> str:
        ...
        # hw15 adds: renewed <copy>, queried credit score
        if self.profile == "public":
            ops = ["borrowed", "ordered", "queried", "read", "graded",
                   "renewed", "credit"]
            weights = [22, 16, 12, 20, 16, 8, 6]
        else:
            ops = ["borrowed", "ordered", "picked", "returned", "queried",
                   "read", "restored", "graded", "renewed", "credit"]
            weights = [18, 12, 10, 10, 8, 12, 10, 10, 6, 4]
        op = self.rng.choices(ops, weights=weights)[0]
        if op == "credit":
            return self.dated(f"{student} queried credit score")
        if op in {"returned", "queried", "restored", "renewed"}:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate HW15 testcases")
    parser.add_argument("--out", default="data_hw15")
