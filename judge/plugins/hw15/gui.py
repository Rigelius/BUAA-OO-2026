...
def discover_projects(co_judge_dir: Path, pattern: str, build_root: Path,
    ...
    official_src = find_official_src(classpath)
    ...
    for idx, (name, project_dir) in enumerate(targets, start=1):
        ...
        projects.append(TargetProject(
            ...
            command=["java", "-cp", f"{build_dir}{cp_sep}{official_src}", main_class],
        ))
    return projects
...
class Validator:
    ...
    def final_checks(self) -> None:
        self.errors.extend(validate_output(self.reference.input_text, "\n".join(self.actual)))
...
def write_summary_report(report_dir: Path, summaries: List[CaseSummary],
    ...
    lines = ["# HW15 Judge Summary", "", f"- total cases: {len(summaries)}",
...
class HW15JudgeWindow(StandardJudgeWindow):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        spec = StandardJudgeSpec(
            ...
            title='OO HW15 评测机',
            base_dir=base_dir,
            self_work_dir=(base_dir.parents[2] / 'U4/hw15').resolve(),
            official_default=(base_dir / 'lib/library3.jar').resolve(),
            fixed_cases_dirname='fixed_cases_hw15',
            official_must_be_file=True,
            official_missing_message='未找到官方包',
            generator_uses_strict_mutual=False,
        generator_seed=20260607,
        generator_limit=260,
...
def build_page(parent: QWidget) -> QWidget:
    return build_standard_page(HW15JudgeWindow, parent)
