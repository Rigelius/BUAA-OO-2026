#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""HW14 flexible legality judge for the OO U4 library task."""
...
DEFAULT_OFFICIAL = JUDGE_DIR / "lib" / "library1.jar"
LOCS = {"bs", "tbs", "bro", "ao", "rr", "user"}
MOVABLE = {"bs", "tbs", "bro", "ao", "rr"}


def _fallback_search_roots() -> List[Path]:
    """Directories to scan when the default library jar path is missing."""
...
def find_official_src(path_text: str = "") -> Path:
    if path_text:
        ...
        if path.is_file() and path.suffix.lower() == ".jar":
            return path.resolve()
        jars = sorted(path.rglob("library1.jar")) if path.is_dir() else []
        if jars:
            return jars[0].resolve()
    if DEFAULT_OFFICIAL.is_file():
        return DEFAULT_OFFICIAL.resolve()
    for base in _fallback_search_roots():
        jars = sorted(base.rglob("library1.jar"))
        if jars:
            return jars[0].resolve()
    raise RuntimeError("cannot locate library1.jar")
...
class FlexibleValidator:
    def __init__(self, input_text: str, output_text: str, hw13_mode: bool = True) -> None:
        # hw13_mode=True: 拒绝 tbs/rr, 拒绝 read/restored/graded, shelf_for 恒等于 "bs"
        self.hw13_mode = hw13_mode
    ...
    def handle_command(self, idx: int, raw: str) -> None:
        ...
        if op == "queried":
            ...
            if self.hw13_mode:
                self.error(f"input command {idx}: 'read' not supported in HW13 baseline: {raw}")
            else:
                self.handle_read(day_s, user, book)
        elif op == "restored":
            if self.hw13_mode:
                self.error(f"input command {idx}: 'restored' not supported in HW13 baseline: {raw}")
            else:
                self.handle_restore(day_s, user, book)
        elif op == "graded" and len(p) == 4:
            if self.hw13_mode:
                self.error(f"input command {idx}: 'graded' not supported in HW13 baseline: {raw}")
            else:
    ...
    def handle_arrange(self, day_s: str, before_open: bool) -> None:
        ...
        for i in range(int(first)):
            ...
            if self.hw13_mode and (frm in {"tbs", "rr"} or to in {"tbs", "rr"}):
                self.error(f"{day_s} arrange: HW13 baseline does not have tbs/rr: {line}")
    ...
    def handle_return(self, day_s: str, user: str, copy_id: str) -> None:
        ...
        verdict, rest = resp
        if rest != copy_id:
            self.error(f"{day_s} returned: target mismatch: {rest}")
        if verdict == "reject":
            if can:
...
def expected_output(input_text: str) -> Tuple[List[str], List[str]]:
    validator = FlexibleValidator(input_text, "", hw13_mode=True)
    validator.parse_input()
    return [], validator.errors


def validate_output(input_text: str, output_text: str) -> List[str]:
    return FlexibleValidator(input_text, output_text, hw13_mode=True).validate()
