...
DEFAULT_OFFICIAL = JUDGE_DIR / "lib" / "library2.jar"
...
def find_official_src(path_text: str = "") -> Path:
    if path_text:
        ...
        if path.is_file() and path.suffix.lower() == ".jar":
            return path.resolve()
        jars = sorted(path.rglob("library2.jar")) if path.is_dir() else []
        if jars:
            return jars[0].resolve()
    if DEFAULT_OFFICIAL.is_file():
        return DEFAULT_OFFICIAL.resolve()
    for base in _fallback_search_roots():
        jars = sorted(base.rglob("library2.jar"))
        if jars:
            return jars[0].resolve()
    raise RuntimeError("cannot locate library2.jar")
