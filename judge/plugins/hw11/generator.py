#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""HW11 testcase generator for the spec3 video-network judge."""
...
MAX_COMMANDS = 3000
FIXED_CASES_DIR = Path(__file__).resolve().parent / "fixed_cases_hw11"
...
def _builtin_fixed_cases() -> List[Tuple[str, List[str]]]:
    return [
        (
            "h11_fixed_01_sample_recommend.txt",
            _normalize_lines(
                """
                add_user 1 20 Alice
                add_user 2 22 Bob
        ...
        (
            "h11_fixed_02_exception_priority.txt",
            _normalize_lines(
                """
                recommend_video 1
                queryGlobalBestContributor
                query_most_influential_up badtype
                add_user 1 20 A
                recommend_video 1
                recommend_Nth_up 1 1
                recommend_Nth_up 1 0
                add_user 1 21 B
                add_user 2 110 Old
                add_user 2 30 B
                add_user_coins 9 5
                ...
                query_user_profile 3
                query_most_popular_video tech
                """
            ),
        ),
        (
            "h11_fixed_03_comments.txt",
            _normalize_lines(
                """
                add_user 1 20 Up
                add_user 2 21 Fan
        ...
        (
            "h11_fixed_04_forward_received_heat.txt",
            _normalize_lines(
                """
                add_user 1 40 Up
                add_user 2 30 Mid
        ...
        (
            "h11_fixed_05_contributor_medal.txt",
            _normalize_lines(
                """
                add_user 1 30 Up
                add_user 2 25 A
        ...
        (
            "h11_fixed_06_longest_and_path.txt",
            _normalize_lines(
                """
                add_user 1 60 U1
                add_user 2 50 U2
        ...
        (
            "h11_fixed_07_profile_and_influence.txt",
            _normalize_lines(
                """
                add_user 1 25 UpA
                add_user 2 26 UpB
                add_user 3 27 Fan
                add_user 4 28 Target
                upload_video 1 701 tech
                upload_video 1 702 music
                upload_video 2 703 tech
                upload_video 2 704 comedy
                watch_video 4 701
                watch_video 4 704
                add_user_coins 4 10
                coin_video 4 701 2
                like_video 4 704
                recommend_video 4
                recommend_Nth_up 4 1
                recommend_Nth_up 4 2
                query_most_influential_up tech
                query_most_influential_up comedy
                query_user_profile 4
                """
            ),
        ),
        ("h11_fixed_08_stress_lis.txt", _stress_lis_case(1200)),
        ("h11_fixed_09_stress_comments.txt", _stress_comments_case(1200)),
    ]
...
class CaseBuilder:
    ...
    def add_user(self, valid: bool = True) -> None:
        ...
        if self.emit(f"add_user {uid} {age} {self.random_name()}"):
            self.users[uid] = age
            self.coins[uid] = 0
    ...
    def like(self, valid: bool = True) -> None:
        ...
        if valid:
            ...
            if vid not in self.watched[uid]:
                if not self.emit(f"watch_video {uid} {vid}"): return
                self.watched[uid].add(vid)
    ...
    def query(self) -> None:
        op = self.rng.choice([
            ...
            "recommend_video", "recommend_Nth_up", "query_most_influential_up",
            "query_user_profile", "queryGlobalBestContributor",
        ])
        if op in {"query_mutual_following_sum", "queryLongestDecSeq", "queryGlobalBestContributor"}:
            self.emit(op)
            ...
            if a is not None and b is not None:
                self.emit(f"{op} {a} {b}")
        elif op in {"query_most_popular_video", "query_most_influential_up"}:
            typ = self.rng.choice(TYPES if self.rng.random() > 0.1 else TYPES + ["badtype"])
            self.emit(f"{op} {typ}")
        elif op == "recommend_Nth_up":
            uid = self.existing_user() if self.rng.random() > 0.1 else self.non_user()
            if uid is not None:
                candidates = [
                    u for u in self.users
                    if u != uid and u not in self.following.get(uid, set())
                ]
                rank = self.rng.randint(1, max(1, len(candidates) + 1))
                if self.rng.random() < 0.1:
                    rank = 0
                self.emit(f"{op} {uid} {rank}")
        elif op in {"recommend_video", "query_user_profile"}:
            uid = self.existing_user() if self.rng.random() > 0.1 else self.non_user()
            if uid is not None:
                if op == "recommend_video" and uid in self.users and not self.watched[uid] and self.videos:
                    vid = self.existing_video()
                    if vid is not None and self.rng.random() > 0.2:
                        self.emit(f"watch_video {uid} {vid}")
                        self.watched[uid].add(vid)
                if op == "query_user_profile" and uid in self.users and not self.watched[uid] and self.videos:
                    vid = self.existing_video()
                    if vid is not None and self.rng.random() > 0.2:
                        self.emit(f"watch_video {uid} {vid}")
                        self.watched[uid].add(vid)
    ...
    def _watched_non_uploader_pair(self) -> Tuple[Optional[int], Optional[int]]:
        ...
        if uid is None or vid is None:
            return None, None
        if vid not in self.watched[uid]:
            if not self.emit(f"watch_video {uid} {vid}"): return None, None
            self.watched[uid].add(vid)
        return uid, vid
...
def generate_all(out_dir: str, random_count: int, base_seed: Optional[int] = None, max_lines: int = 3000) -> List[str]:
    ...
    for i in range(max(0, random_count)):
        ...
        filename = f"h11_random_{i + 1:02d}.txt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate HW11 testcases")
    parser.add_argument("--out", default="data_hw11")
