...
public interface NetworkInterface {
    ...
    /*@ invariant (\forall int i,j; 0 <= i && i < j && j < users.length; !users[i].equals(users[j])) &&
      @  (\forall int i,j; 0 <= i && i < j && j < videos.length; !videos[i].equals(videos[j]))
      @*/
    ...
    /*@ public normal_behavior
      ...
      @ ensures getUser(id).videos.length == 0;
      @ ensures (\forall int i; 0 <= i && i < getUser(id).typeCounts.length;
      @              getUser(id).typeCounts[i] == 0);
    ...
    /*@ public normal_behavior
      ...
      @ assignable videos, users[*].receivedVideos, users[*].videos;
      ...
      @ ensures (\exists int i; 0 <= i && i < getUser(uploaderId).videos.length;
@              getUser(uploaderId).videos[i].getId() == videoId);
    ...
    /*@ public normal_behavior
      ...
      @ ensures (\exists int idx; 0 <= idx < getUser(userId).types.length;
      @             getVideo(videoId).getType().equals(getUser(userId).types[idx]) &&
      @                 getUser(userId).typeCounts[idx] == \old(getUser(userId).typeCounts[idx]) + 1);
      @ also
      @ public exceptional_behavior
    ...
    /*@ public normal_behavior
      ...
      @ ensures getUser(userId).likedVideos.length == \old(getUser(userId).likedVideos.length) + 1;
      @ ensures (\forall int i; 0 <= i && i < \old(getUser(userId).likedVideos.length);
      @             getUser(userId).likedVideos[i].equals(\old(getUser(userId).likedVideos[i])));
      @ ensures getUser(userId).likedVideos[getUser(userId).likedVideos.length - 1].equals(getVideo(videoId));
      ...
      @ ensures getUser(userId).likedVideos.length == \old(getUser(userId).likedVideos.length) - 1;
      @ ensures (\forall int i; 0 <= i && i < \old(getUser(userId).likedVideos.length);
      @             !\old(getUser(userId).likedVideos[i]).equals(getVideo(videoId)) ==>
      @             (\exists int j; 0 <= j && j < getUser(userId).likedVideos.length;
      @                 getUser(userId).likedVideos[j].equals(\old(getUser(userId).likedVideos[i]))));
    ...
    /*@ public normal_behavior
      ...
      @ ensures getUser(getVideo(videoId).getUploaderId()).contributions.length == \old(getUser(getVideo(videoId).getUploaderId()).contributions.length) + 1;
      @ ensures (\forall int j; 0 <= j && j < \old(getUser(getVideo(videoId).getUploaderId()).contributors.length);
      @             getUser(getVideo(videoId).getUploaderId()).contributors[j].equals(\old(getUser(getVideo(videoId).getUploaderId()).contributors[j])) &&
      @             getUser(getVideo(videoId).getUploaderId()).contributions[j] == \old(getUser(getVideo(videoId).getUploaderId()).contributions[j]));
      ...
      @ ensures getUser(userId).getCoins() == \old(getUser(userId).getCoins()) - amount;
      @ ensures getUser(getVideo(videoId).getUploaderId()).getCoins() == \old(getUser(getVideo(videoId).getUploaderId()).getCoins()) + amount;
      @ ensures getUser(getVideo(videoId).getUploaderId()).contributors.length == \old(getUser(getVideo(videoId).getUploaderId()).contributors.length);
      @ ensures getUser(getVideo(videoId).getUploaderId()).contributions.length == \old(getUser(getVideo(videoId).getUploaderId()).contributions.length);
      @ ensures (\forall int i; 0 <= i && i < getUser(getVideo(videoId).getUploaderId()).contributors.length;
      @             getUser(getVideo(videoId).getUploaderId()).contributors[i].equals(\old(getUser(getVideo(videoId).getUploaderId()).contributors[i])));
      @ ensures (\forall int i; 0 <= i && i < getUser(getVideo(videoId).getUploaderId()).contributors.length;
      @             (!getUser(getVideo(videoId).getUploaderId()).contributors[i].equals(getUser(userId))) ==>
      @             (getUser(getVideo(videoId).getUploaderId()).contributions[i] == \old(getUser(getVideo(videoId).getUploaderId()).contributions[i])));
      @ ensures (\exists int i; 0 <= i && i < getUser(getVideo(videoId).getUploaderId()).contributors.length;
      @             getUser(getVideo(videoId).getUploaderId()).contributors[i].equals(getUser(userId)) &&
      ...
      @*/
    public /*@ safe @*/ void coinVideo(int userId, int videoId, int amount) throws UserIdNotFoundException, VideoIdNotFoundException, EqualUserIdException, InsufficientCoinsException, VideoUnwatchedException, InvalidCoinsException;

    /*@ public normal_behavior
      @ requires containsUser(id) && getUser(id).contributors.length > 0;
      @ assignable \nothing;
      @ ensures \result == (\min int bestId;
      @                         (\exists int i; 0 <= i && i < getUser(id).contributors.length; getUser(id).contributors[i].getId() == bestId &&
    ...
    /*@ public normal_behavior
      @ requires containsUser(userId) && containsUser(followerId) && containsVideo(videoId) &&
      @          getUser(userId).hasWatchedVideo(getVideo(videoId)) &&
      @          getUser(userId).containsFollower(getUser(followerId));
      ...
      @ ensures (\forall int i; 0 <= i && i < \old(getUser(followerId).receivedVideos.length);
      @           getUser(followerId).receivedVideos[i + 1] == \old(getUser(followerId).receivedVideos[i]));
    ...
    /*@ public normal_behavior
      ...
      @ ensures getVideo(videoId).commentIds.length == \old(getVideo(videoId).commentIds.length) + 1;
    ...
    /*@ public normal_behavior
      @ requires users.length > 0 && (\forall int i; 0 <= i && i < users.length; users[i].contributors.length == 0);
      @ assignable \nothing;
      @ ensures \result != null && \result.length == 2;
      @ ensures \result[0] == 0 && \result[1] == 0;
      @ also
      @ public normal_behavior
      @ requires users.length > 0 && (\exists int i; 0 <= i && i < users.length; users[i].contributors.length > 0);
      @ assignable \nothing;
      @ ensures \result != null && \result.length == 2;
      @ ensures \result[1] == (\max int id;
      @             (\exists int k; 0 <= k && k < users.length && users[k].contributors.length > 0; queryBestContributor(users[k].getId()) == id);
      @             (\num_of int i; 0 <= i && i < users.length && users[i].contributors.length > 0; queryBestContributor(users[i].getId()) == id));
      @ ensures \result[0] == (\min int id;
      @             (\exists int k; 0 <= k && k < users.length && users[k].contributors.length > 0; queryBestContributor(users[k].getId()) == id) &&
      @             (\num_of int i; 0 <= i && i < users.length && users[i].contributors.length > 0; queryBestContributor(users[i].getId()) == id) == \result[1];
      @             id);
      @ also
      @ public exceptional_behavior
      @ signals (NoUserException e) users.length == 0;
      @*/
    public /*@ pure @*/ int[] queryGlobalBestContributor() throws NoUserException;
    
    /*@ public normal_behavior
      @ requires containsUser(userId) && videos.length > 0 && getUser(userId).watchedVideos.length > 0;
      @ assignable \nothing;
      @ ensures (\result == (\min int bestId;
      @                         (\exists int i; 0 <= i && i < videos.length; videos[i].getId() == bestId &&
      @                             (\forall int j; 0 <= j && j < videos.length;
      @                                 computeVideoScore(getUser(userId), videos[j]) <= computeVideoScore(getUser(userId), videos[i]));
      @                         bestId));
      @ also
      @ public exceptional_behavior
      @ signals (UserIdNotFoundException e) !containsUser(userId);
      @ signals (NoVideoUploadedException e) containsUser(userId) && videos.length == 0;
      @ signals (ColdStartVideoException e) containsUser(userId) && videos.length > 0 && getUser(userId).watchedVideos.length == 0;
      @*/
    public /*@ pure @*/ int recommendVideo(int userId) throws UserIdNotFoundException, NoVideoUploadedException, ColdStartVideoException;

    // @ensures \result == (video.getHeat() * user.getInterest(video.getType(), videos.length));
    public /*@ pure @*/ long  computeVideoScore(UserInterface user, VideoInterface video);
    ...
    /*@ public normal_behavior
      ...
      @ public exceptional_behavior
      @ signals (UserIdNotFoundException e) !containsUser(userId);
      @ signals (InvalidRankException e) containsUser(userId) && rank <= 0;
      @ signals (NoVideoUploadedException e) containsUser(userId) && rank > 0 && videos.length == 0;
      @ signals (ColdStartUserException e) containsUser(userId) && rank > 0 && videos.length > 0 &&
      @         (\num_of int i; 0 <= i && i < users.length; users[i].getId() != userId && !getUser(userId).isFollowing(users[i])) < rank;
      @*/
    public /*@ pure @*/ int recommendNthUp(int userId, int rank) throws UserIdNotFoundException, InvalidRankException, NoVideoUploadedException, ColdStartUserException;

    /*@ public normal_behavior
      @ requires isValidType(type);
      @ assignable \nothing
      @ ensures (\result == (\min int bestId;
      @                         (\exists int i; 0 <= i && i < users.length; users[i].id == bestId &&
      @                             (\forall int j; 0 <= j && j < users.length; getUser(bestId).getInfluence(type) >= users[j].getInfluence(type));
      @                         bestId));
      @ also
      @ public exceptional_behavior
      @ signals (InvalidTypeException e) !isValidType(type);
      @ signals (NoUserException e) users.length == 0;
     */
    public /* pure */ int queryMostInfluentialUp(String type) throws InvalidTypeException, NoUserException;

    /*@ public normal_behavior
      @ requires containsUser(userId) && !getUser(userId).watchedVideos.isEmpty();
      @ assignable \nothing
      @ ensures \result == getUser(userId).getProfile(videos.length);
      @ also
      @ public exceptional_behavior
      @ signals (UserIdNotFoundException e) !containsUser(userId);
      @ signals (ColdStartVideoException e) containsUser(userId) && getUser(userId).watchedVideos.isEmpty();
     */
    public /* pure */ List<Integer> queryUserProfile(int userId) throws UserIdNotFoundException, ColdStartVideoException;
}