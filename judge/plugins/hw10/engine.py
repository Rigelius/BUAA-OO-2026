#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""HW10 reference-output judge for OO U3 spec2."""
...
def find_official_src(path_text: str = "") -> Path:
    ...
    for base in _fallback_search_roots():
        try:
            candidates = list(base.rglob("面向对象第三单元第二次作业官方包"))
        except OSError:
            continue
    ...
    for base in _fallback_search_roots():
        try:
            candidates = list(base.rglob("NetworkInterface.java"))
        except OSError:
            continue
        for candidate in candidates:
            try:
                if "spec2" in candidate.read_text(encoding="utf-8", errors="replace"):
                    return candidate.parents[3].resolve()
            except OSError:
                pass
    raise RuntimeError("cannot locate official spec2 source directory")
...
class ReferenceModel:
    ...
    def unfollow_user(self, a: int, b: int) -> None:
        if a not in self.users:
            ...
            self.out.append(self.err.pair("flnf", a, b))
        else:
            if a in self.following[b]:
                self.mutual -= 1
    ...
    def forward_video(self, uid: int, vid: int, follower: int) -> None:
        if uid not in self.users:
            ...
            self.out.append(self.err.pair("flnf", uid, follower))
        else:
    ...
    def heat(self, vid: int) -> Decimal:
        video = self.videos[vid]
        return (Decimal(int(video["play"])) + Decimal(int(video["likes"])) * Decimal("1.5")
                + Decimal(int(video["forward"])) * Decimal("2.0")
                + Decimal(int(video["coins"])) * Decimal("2.5"))
...
def write_summary(report_dir: Path, summaries: List[dict]) -> None:
    ...
    lines = ["# HW10 Judge Report", "", f"- total_cases: {len(summaries)}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HW10 reference-output judge")
    ...
    parser.add_argument("--build-dir", default=".build_targets_hw10")
    parser.add_argument("--generate", action="store_true")
    parser.add_argument("--count", type=int, default=20)
    parser.add_argument("--seed", type=int, default=20260515)
    parser.add_argument("--max-lines", type=int, default=3000)
    parser.add_argument("--data-dir", default="data_hw10")
    parser.add_argument("--out-dir", default="out_raw_hw10")
    parser.add_argument("--report-dir", default="report_hw10")
    parser.add_argument("--problem-dir", default="problem_cases_hw10")
